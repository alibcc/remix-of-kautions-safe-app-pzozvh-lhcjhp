 ...
 * Handles FormData properly for image uploads
 */
export const uploadImage = async (imageUri: string, filename?: string): Promise<{ url: string; filename: string }> => {
  const token = await getBearerToken();
  ...
  const name = filename || imageUri.split('/').pop() || 'photo.jpg';
  ...
  try {
    ...
    if (!response.ok) {
      const text = await response.text();
      console.error("[API] Image upload error:", response.status, text);
      throw new Error(`Image upload failed: ${response.status} - ${text}`);
    }

    const data = await response.json();
    console.log("[API] Image uploaded successfully:", data);
    return data;
  } catch (error) {
